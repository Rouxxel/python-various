...
A minimal Chrome extension (Manifest V3) that opens a **side panel** when you click the toolbar icon. The panel stays open while you browse. Default UI: logo and a click counter. Optional backend connectivity.

## Features

- **Manifest V3** with side panel (no popup)
- **Vanilla HTML, CSS, and JavaScript** – no build step
- **Click counter** – persisted in `chrome.storage.local` so it survives closing the panel
...
```
sidepanel_template/
├── manifest.json     # MV3: side_panel.default_path, storage, sidePanel, background
├── popup.html        # Panel UI: logo, count display, Count button
...
2. **Load in Chrome**:
   ...
   - Click **Load unpacked**
   - Select the `sidepanel_template` folder
3. Click the extension icon in the toolbar to open the side panel. Use **Count** to increment; the value is saved across opens. The panel stays open until you close it.

## Connecting to a Backend

To call your own API (e.g. REST) from the panel:
...
2. **Set the API base URL** in one of two ways:
   ...
   - At runtime: open the side panel, open DevTools (right‑click panel → Inspect), then in the console run:
     ```js
     chrome.storage.local.set({ backendBaseUrl: 'http://localhost:8000' });
     ```
     Reload the panel; the "Ping backend" section will appear.
...
- **UI**: Edit `popup.html`, `popup.css`, and `popup.js` to build your panel UI and logic.

## Requirements

- Google Chrome (or Chromium) with Manifest V3 and side panel support.
